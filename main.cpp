...
void usage() {
	printf("Usage\n");
	printf("fbblock [bvf_map_file] [parameter file]\n");
	printf("fbblock solid_[m]_[n]_[p] [parameter_file]\n");
}

bool define_bvfmap(FBArray3D<unsigned char> &bvfmap,QString bvfmap_definition_string) {
	QStringList vals=bvfmap_definition_string.split("_");
	if (vals.count()!=4) return false;
	if (vals[0]=="solid") {
		int m=vals[1].toInt();
		int n=vals[2].toInt();
		int p=vals[3].toInt();
		if (m*n*p==0) return false;
		bvfmap.allocate(m,n,p);
		bvfmap.setAll(100);
		return true;
	}
	else return false;
...
int main(int argc, char *argv[])
{
	...
	QString param_fname;
	QString base_fname;
	QString bvfmap_definition_string;
	{
		for (int i=1; i<argc; i++) {
			...
			else if (param.startsWith("solid_")) {
				bvfmap_definition_string=param;
			}
		}
		if (!bvfmap_fname.isEmpty())
			base_fname=QFileInfo(bvfmap_fname).path()+"/"+QFileInfo(bvfmap_fname).baseName();
		else
			base_fname=QFileInfo(param_fname).path()+"/"+QFileInfo(param_fname).baseName();
		#ifdef WIN32
	...
	if ((bvfmap_fname.isEmpty())||(param_fname.isEmpty())) {
		if (bvfmap_definition_string.isEmpty()) {
			usage(); //print usage information
			return -1;
		}
	}
	...
	FBBlockSolver Solver;
	{
		...
		if (!bvfmap_definition_string.isEmpty()) {
			if (!define_bvfmap(bvfmap,bvfmap_definition_string)) {
				usage();
				return -1;
			}
		}
	...
	{	
		...
		Solver.setYoungsModulus(youngs_modulus);
		Solver.setVoxelVolume(resolution[0]*resolution[1]*resolution[2]);
	}
	...
	if (PF.getString("COMPRESSION TEST")=="yes") {
		...
		if (!initial_displacements_fname.isEmpty()) {
			...
			Solver.setInitialDisplacementsOnFreeVariables(displacements0);
		}
		
		if (PF.getString("NONLINEAR_STEP_SIZE").toFloat()==0) {
			...
			float step_size=PF.getString("NONLINEAR_STEP_SIZE").toFloat();
			int num_steps=PF.getString("NONLINEAR_NUM_STEPS").toInt();
			int num_iterations_per_step=PF.getString("NONLINEAR_NUM_ITERATIONS_PER_STEP").toInt();
			printf("Performing compression test in %s direction...\n",dir.toAscii().data());		
			Solver.solveNonlinear(step_size,num_steps,num_iterations_per_step);
			Solver.clear();
			printf("Done.\n");
		}
	}
